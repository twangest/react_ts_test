...
const FromPage: React.FunctionComponent<IFromPageProps> = (props) => {
  ...
  useEffect(() => {
    const fetchData = async (requestParams: RequestParamType) => {
      ...
      const res:HttpResponse<object> = await postFormData(API_URL, formData);
      setLoading(false);
      setLoaded(true);
  ...
  return (
    <div className='row'>
      <form className='col s6' onSubmit={handleSubmit}>
        ...
        <FormInputText id='patronymic' label='Отчество' className='white-text' value={patronymic} onChange={setPatronymic} />
